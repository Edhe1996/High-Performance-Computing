...
cl_device_id selectOpenCLDevice()
{
  ...
  cl_device_id device_index;
  cl_platform_id platforms[8];
  cl_device_id devices[MAX_DEVICES];
  char name[MAX_DEVICE_NAME];

  err = clGetPlatformIDs(0, NULL, &numPlatforms);
  checkError(err, "Finding platforms", __LINE__);
  if (numPlatforms == 0)
  {
  ...
  {
      err = clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_GPU, 1, &device_index, NULL);
      if (err == CL_SUCCESS)
      {
          break;
      }
  ...
  if (device_index == NULL)
      checkError(err, "Finding a device", __LINE__);

  err = output_device_info(device_index);
  checkError(err, "Printing device output", __LINE__);
